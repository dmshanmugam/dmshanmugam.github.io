...
[^1] To transcribe tables of results in LaTex, use the pandas `to_latex()` function! To transcribe tables of results into a Google Doc, use the `to_markdown()` function. Copy the outputted text, right-click at the desired location, and select 'Paste from Markdown'.